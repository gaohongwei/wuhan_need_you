...
class NoticeModelView(ModelView):
    ...
    form_overrides = dict(
        content=CKEditorField, priority=SelectField, status=SelectField
    )
    form_choices = {"type": [("信息", "信息"), ("广告", "广告")]}
    form_args = dict(
        priority=dict(
            choices=[(0, "普通"), (1, "优先"), (2, "紧急")], coerce=int, label="优先级"
        ),
        status=dict(
            choices=[(0, "草稿"), (1, "已提交"), (2, "已审核"), (3, "未批准")],
            coerce=int,
            label="状态",
        ),
    )
    ...
    column_formatters = {
        ...
        "_created_time": lambda v, c, m, n: format_cn(
            m.created_time
        ),  # a hack for details form
        "_modified_time": lambda v, c, m, n: format_cn(
            m.modified_time
        ),  # a hack for details form
        "_permitted_time": lambda v, c, m, n: format_cn(
            m.permitted_time
        ),  # a hack for details form
    }
    ...
    def action_approve(self, ids):
        try:
            rows = (
                Notice.query.filter(Notice.id.in_(ids))
            ...
            flash(
                gettext("Failed to approve notices. %(error)s", error=str(ex)), "error"
            )

    @action(
        "disapprove",
        "Disapprove",
        "Are you sure you want to disapprove selected notices?",
    )
    def action_disapprove(self, ids):
        try:
            ...
            flash(
                gettext("Failed to disapprove notices. %(error)s", error=str(ex)),
                "error",
            )
