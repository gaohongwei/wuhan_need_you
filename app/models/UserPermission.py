...
class RoutePermission(UserPermission):
    ...
    def allow(self, user):
        ...
        return (
            allow_role_name != None
            and allow_role_name(self.role_name)
            and self.match_route(self.route)
        )
...
def register_model_view_permission(modelViewClass, role_name):
    model_view_permissions[modelViewClass] = ModelViewPermission(
        modelViewClass, role_name
    )
