...
class Comment(db.Model):
	...
	def add(cls):
		...
		c = Comment(name=name, email=email, comment=comment, notice_title=notice_title, notice_id=notice_id)
		db.session.add(c)
		db.session.commit()
	...
	@classmethod
	def get_comment_by_noticeid(cls, notice_id):
		return db.session.query(Comment).filter_by(notice_id=notice_id).order_by(Comment.created_time.desc()).all()